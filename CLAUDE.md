...
- **`represented_ids`** (`string[]`, init. `[card.id]`) — IDs de cartes que l'unité « représente » pour le matching de matériaux fusion/heritage/transformation. Une **Transformation** hérite toujours des `represented_ids` de l'unité remplacée (1 seule cible). Une **Fusion**/**Heritage** n'hérite des `represented_ids` de son matériau consommé que si **un seul** matériau a été consommé (`[card.id, ...materiau.represented_ids]`) — au-delà de 1, la lignée est considérée diluée et l'unité ne représente plus que sa propre carte (`[card.id]`). Exemple : Aile de feu, fusionnée à partir d'Avian + un autre matériel, ne peut pas remplacer Avian dans une fusion ultérieure (`InvocationManager._inheritedRepresentedIds`). Affiché dans le tooltip d'unité (`Tooltip.unitHtml`, icône 🧬) quand la lignée dépasse la carte de base.
